...
## Email Dashboard (Assignment Week Focus - Mock API)
...
- **DASH-02**: As a user, I can select a folder from the left sidebar so I can view emails/threads in that folder.
- **DASH-03**: As a user, I can see a list of threads with sender, subject, preview, and timestamp so I can quickly scan my inbox.
- **DASH-04**: As a user, I can select a thread from the list so I can view its full content with all messages in the detail pane.
- **DASH-05**: As a user, I can see thread actions (Reply, Forward, Delete, Star) so I can manage my emails (UI mockup only, no actual operations in MVP).
- **DASH-06**: As a user, I can navigate threads using keyboard shortcuts so I can work efficiently.
...
- **DASH-08**: As a user, threads are loaded efficiently using virtual scrolling and caching so the app remains performant with large mailboxes.
